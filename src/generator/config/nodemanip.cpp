...
int addNodes(std::string link, std::vector<Proxy> &allNodes, int groupID, parse_settings &parse_set)
{
    ...
    {
        custom_headers.emplace("User-Agent", *parse_set.custom_user_agent);
        writeLog(LOG_TYPE_INFO, "Using custom User-Agent: " + *parse_set.custom_user_agent);
    } else {
        // 如果没有自定义UA，使用clash.meta UA
        custom_headers.emplace("User-Agent", "clash.meta");
        writeLog(LOG_TYPE_INFO, "Using default User-Agent: clash.meta");
    }
